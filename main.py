...
def random_check():
    ...
    ID, DescID, Mvir, Vmax, Vrms, Rvir, Rs, Np, x, y, z, VX, VY, VZ, JX, JY, JZ, Spin, rs_klypin, Mvir_all, M200b, M200c, M500c, M2500c, Xoff, Voff, spin_bullock, b_to_a, c_to_a, A_x_, A_y_, A_z_, b_to_a_500c_, c_to_a_500c_, A_x__500c_, A_y__500c_, A_z__500c_, TU, M_pe_Behroozi, M_pe_Diemer = \
        loadtxt(path+"/rockstar/out_{:02d}.list".format(64-2), unpack=True)
    ...
    for i,c in enumerate(center):
        if Rvir[i]>400 and Rvir[i]<500:
            print(i)
            track_list = track_run(os.environ["data"]+"\\ds14_scivis_0128\\raw",64,67,1,c,0.5,2,model,device,args.dim,True)
            truth_list = get_benchmark(os.environ["data"]+"\\ds14_scivis_0128",64,67,ID[i])
...
if __name__ == "__main__":
    ...
    data_file = "./data/cosmology_ball.npy"
    ...
    if args.phase == 1:
        ...
        random_check()
        # track_list = track_run(os.environ["data"]+"\\ds14_scivis_0128\\raw",64,67,1,(2.76107, 30.67916, 11.95021),0.5,2,model,device,args.dim,True)
